...
class PointTransformerCls(nn.Module):
    def __init__(self, cfg, output_channels=40):
        ...
        self.transformer0 = EncoderLayer(
            n_points = 1024, 
            hidden_size = 64, 
            ffn_size = 16, 
            dropout_rate = 0.1, 
            ...
            num_heads = 8)
        self.transformer1 = EncoderLayer(n_points = 1024, hidden_size = 64, ffn_size = 16, 
            dropout_rate = 0.1, 
            attention_dropout_rate = 0.1, 
            num_heads = 8)
        self.transformer2 = EncoderLayer(n_points = 1024, hidden_size = 64, ffn_size = 16, 
            dropout_rate = 0.1, 
            attention_dropout_rate = 0.1, 
            num_heads = 8)

        self.transformer3 = EncoderLayer(n_points = 1024, hidden_size = 128, ffn_size = 16, 
            dropout_rate = 0.1, 
            attention_dropout_rate = 0.1, 
            num_heads = 8)
        self.transformer4 = EncoderLayer(n_points = 1024, hidden_size = 256, ffn_size = 16, 
            dropout_rate = 0.1, 
            attention_dropout_rate = 0.1, 
            num_heads = 8)
    ...
    def forward(self, x):
        ...
        # a = self.softmax(x1) 
        # x1 = torch.einsum('bhi,bfi->bihf', a, x0)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_bias=None):
        ...
        x = F.leaky_relu(self.output_layer(x), negative_slope=0.2)
...
class EncoderLayer(nn.Module):
    def __init__(self, n_points, hidden_size, ffn_size, dropout_rate, attention_dropout_rate, num_heads):
        ...
        self.self_attention_norm = nn.LayerNorm([n_points, hidden_size])
        self.self_attention = MultiHeadAttention(
            hidden_size, attention_dropout_rate, num_heads)
        self.self_attention_dropout = nn.Dropout(dropout_rate)

        self.ffn_norm = nn.LayerNorm([n_points, hidden_size])

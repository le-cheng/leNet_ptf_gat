...
def get_graph_feature(x, k=20, idx=None):
    ...
    # feature = (x+x-feature).permute(0, 3, 1, 2).contiguous() 
    return feature
...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, attention_dropout_rate, num_heads):
        ...
        self.att_size = att_size = 8
        # hidden_size // num_heads
        assert att_size > 0
        self.scale = torch.rsqrt(torch.Tensor([att_size])).cuda()
        self.dim_qkv = att_size
        ...
        self.output_layer = nn.Sequential(
            nn.Conv1d(num_heads * att_size , hidden_size, kernel_size=1, bias=False),
            # nn.Linear(num_heads * att_size, hidden_size),
    ...
    def forward(self, q, k, v, attn_bias=None):
        ...
        q = self.linear_q(q).view(batch_size, -1, self.num_heads, self.dim_qkv)
        k = self.linear_k(k).view(batch_size, -1, self.num_heads, self.dim_qkv)
        v = self.linear_v(v).view(batch_size, -1, self.num_heads, self.dim_qkv)

        # q = q.transpose(3, 2)  
        q = q.permute(0, 2, 1, 3)                # [b, h, q_len, d_k]
        # v = v.transpose(3, 2)     
        v = v.permute(0, 2, 1, 3)                # [b, h, v_len, d_v]
        k = k.permute(0, 2, 3, 1)                # [b, h, d_k, k_len]
        ...
        # x = self.att_dropout(x)# 去除这个dropout有效提高模型拟合速度
        x = x.matmul(v)  # [b, h, q_len, attn]

        x = x.transpose(3, 2).contiguous()  # [b, q_len, h, attn]
        x = x.view(batch_size, self.num_heads * self.dim_qkv, -1)

        x = self.output_layer(x).transpose(1, 2).contiguous()
